...
public class VodConfig {
    ...
    private void initParse() {
        String mainConfig = Hawk.get(HawkAdm.ADM_CONFIG, "");
        if (mainConfig != null && !TextUtils.isEmpty(mainConfig)) {
            JsonObject mainJson = App.gson().fromJson(mainConfig, JsonObject.class);
            JsonObject mainData = App.gson().fromJson(mainJson.get("data"), JsonObject.class);
            for (JsonElement element : mainData.get("parsesConfig").getAsJsonArray()) {
                JsonObject parseConfig = element.getAsJsonObject();
                String url = parseConfig.get("url").getAsString();  //获得接口地址
                String name = parseConfig.get("name").getAsString(); //获得接口名称
                if (!url.equals("auto") && !name.equals("超级解析")) { //不添加超级解析配置
                    parseConfig.remove("status");
                    parseConfig.remove("type_text");
                    parseConfig.remove("status_text");
                    if (parseConfig.has("ext")) {
                        String ext = parseConfig.get("ext").getAsString();
                        String newExt = new String(Base64.decode(ext, Base64.DEFAULT));
                        parseConfig.remove("ext");
                        parseConfig.add("ext", JsonParser.parseString(newExt));
                    }
                    Parse parse = Parse.objectFrom(parseConfig);
    ...
    private void initSite(JsonObject object) {
        ...
        for (JsonElement element : Json.safeListElement(object, "sites")) {
            ...
            if (sites.contains(site) || Utils.isRemove(site.getName(), HawkAdm.hideSite())) {
                continue;
            }
    ...
    private void initParse(JsonObject object) {
        for (JsonElement element : Json.safeListElement(object, "parses")) {
            ...
            if (Utils.isRemove(parse.getName(), HawkAdm.hideParse())) continue;
    ...
    private void initOther(JsonObject object) {
        if (parses.size() > 0) {
            ...
            if (mainConfig != null && !TextUtils.isEmpty(mainConfig)) {
                ...
                for (JsonElement element : mainData.get("parsesConfig").getAsJsonArray()) {
                    JsonObject parseConfig = element.getAsJsonObject();
                    String url = parseConfig.get("url").getAsString();
                    String name = parseConfig.get("name").getAsString();
                    if (url.equals("auto") && name.equals("超级解析")) {
