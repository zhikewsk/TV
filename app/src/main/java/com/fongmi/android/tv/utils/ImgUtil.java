...
public class ImgUtil {
    ...
    public static void load(String text, int error, String url, ImageView view) {
        load(text, error, url, view, ImageView.ScaleType.FIT_XY);
    }
    ...
    public static void load(String text, int error, String url, ImageView view, ImageView.ScaleType scaleType) {
        view.setScaleType(scaleType);
        if (!TextUtils.isEmpty(url)) Glide.with(App.get()).asBitmap().load(getUrl(url)).placeholder(R.drawable.ic_img_loading).skipMemoryCache(true).dontAnimate().sizeMultiplier(Setting.getThumbnail()).signature(new ObjectKey(url + "_" + Setting.getQuality())).listener(getListeners(view, scaleType)).into(view);
        else if (text.length() > 0) view.setImageDrawable(getTextDrawable(text.substring(0, 1), true));
        else view.setImageResource(error);
    }
    ...
    public static void loadAvatar(String url, ImageView view) {
        view.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
        if (TextUtils.isEmpty(url)) view.setImageResource(R.drawable.user_avatar);
        else Glide.with(App.get()).asBitmap().load(getUrl(url)).apply(RequestOptions.circleCropTransform()).error(R.drawable.user_avatar).placeholder(R.drawable.user_avatar).listener(getListener(view)).into(view);
    }
    ...
    private static RequestListener<Bitmap> getListeners(ImageView view, ImageView.ScaleType scaleType) {
        return new RequestListener<>() {
            @Override
            public boolean onLoadFailed(@Nullable GlideException e, Object model, @NonNull Target<Bitmap> target, boolean isFirstResource) {
                view.setImageResource(R.drawable.ic_img_error);
